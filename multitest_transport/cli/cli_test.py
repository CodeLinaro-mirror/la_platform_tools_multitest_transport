...
class CliTest(parameterized.TestCase):
  ...
  def testStart_connectLabserverToConfigServer(self):
    """Test start with connect_labserver_to_config_server."""
    args = self.arg_parser.parse_args([
        'start',
    ...
    self.assertIn(
        'MTT_CONNECT_LABSERVER_TO_CONFIG_SERVER=true',
        docker_create_args,
    )
  ...
  def testStart_mountHostAndroidDir(self):
    args = self.arg_parser.parse_args(['start', '--mount_host_android_dir'])
    cli.Start(args, self._CreateHost(cluster_name='acluster'))
